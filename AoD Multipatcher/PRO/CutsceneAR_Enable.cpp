...
void CutsceneAR_Enable()							// DA SISTEMARE TUTTO
{
	
	// .data:0080FC80 _gCameraAspectRatio
	...
	Pro_Fix_Class exe = Pro_Fix_Allocate_space(9000, Title);
	system("cls");
	cout << "Allocazione bytes...";
	Sleep(2000);
	// Conversione offset codice assembly in stringa
	//string offset = "fill";
	//memcpy(&offset, &exe.Virtual_offset, 4);
	//// Scrittura blocco 1 (".\Data\Effects\Font.tga")
	//string temp = EXEorig.substr(0, exe.EXE_offset);
	//temp += code1;
	//temp += EXEorig.substr(exe.EXE_offset + code1.size(), string::npos);
	//EXEorig = temp;
